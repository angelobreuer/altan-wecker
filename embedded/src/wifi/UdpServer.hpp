...
namespace wifi {
static constexpr const std::array<uint8_t, 7> UdpPacketHeader = {
    {0x88, 0x86, 0x96, 0x77, 0x7F, 0x7F, 0x66}};
...
class UdpServer {
  public:
    ...
    UdpServer(UdpProcessor *processor)
        : _task{nullptr}, _processor{processor} {}
  ...
  private:
    ...
    static void udp_server_task(void *pvParameters) {
        ...
        while (1) {
            ...
            while (1) {
                auto bytesReceived =
                    recvfrom(socketHandle.GetHandle(), receiveBuffer.data(),
                ...
                if (bytesReceived < 12 ||
                    strncmp((const char *)UdpPacketHeader.data(),
                            (const char *)receiveBuffer.data(),
                            UdpPacketHeader.size()) != 0) {
                ...
                uint32_t correlationId =
                    *reinterpret_cast<uint32_t *>(&receiveBuffer.data()[8]);
                ...
                ResponseContext response = {
                    ...
                    .operationStatus = OperationStatus::kSuccess,
                    .length = 0,
                };
                ...
                auto sendResult = sendto(
                    socketHandle.GetHandle(), sendBuffer.data(),
                    response.length + 12, 0, (struct sockaddr *)&source_addr,
                    sizeof(source_addr));
