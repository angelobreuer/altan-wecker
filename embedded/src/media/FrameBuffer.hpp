...
namespace media {
template <size_t UnitSize> struct FrameBufferWriter {
    FrameBufferWriter(QueueHandle_t availableBufferQueue,
                      QueueHandle_t filledBufferQueue)
        : _availableBufferQueue{availableBufferQueue}, _filledBufferQueue{
                                                           filledBufferQueue} {}

    std::array<uint8_t, UnitSize> *Rent() {
        std::array<uint8_t, UnitSize> *buffer;

        while (xQueueReceive(_availableBufferQueue, &buffer, portMAX_DELAY) !=
               pdPASS) {
        }
...
template <size_t UnitSize> struct FrameBufferReader {
    FrameBufferReader(QueueHandle_t availableBufferQueue,
                      QueueHandle_t filledBufferQueue)
        : _availableBufferQueue{availableBufferQueue}, _filledBufferQueue{
                                                           filledBufferQueue} {}
    ...
    std::array<uint8_t, UnitSize> *Wait() {
        ...
        while (xQueueReceive(_filledBufferQueue, &buffer, portMAX_DELAY) !=
               pdPASS) {
        }
...
template <size_t UnitSize, size_t Capacity> class FrameBuffer {
  public:
    FrameBuffer() {
