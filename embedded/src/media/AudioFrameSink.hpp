...
class AudioFrameSink
    : public alarm_clock::media::FrameSink<SampleRate * Channels *
                                           BytesPerSample> {
  public:
    AudioFrameSink()
        : _sampleCounter{0}, _buffer{nullptr}, _playoutSemaphoreHandle{
                                                   nullptr} {
        ...
        ESP_ERROR_CHECK(gptimer_register_event_callbacks(
            gptimer, &timerEventCallbacks, this));
  ...
  private:
    ...
    DoProcessInterrupt(gptimer_handle_t timer,
                       const gptimer_alarm_event_data_t *edata,
                       void *frameSink) {
        auto instance =
            static_cast<AudioFrameSink<SampleRate, Channels, BytesPerSample> *>(
                frameSink);
